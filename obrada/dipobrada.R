...
# korelacija implicitnih i eksplicitnih mjera
# cor(rezmat[, c(2, 8:11, 15, 39)], method = 'pearson', use = 'pairwise')
corrmat <- cor.ci(rezmat[, c(2, 8:11, 15, 39)], n.iter = 1000, method = 'pearson')

# corrmat$rho %>%
#     round(digits = 2) %>%
#     latex(file = '', booktabs = TRUE, numeric.dollar = FALSE, where = '!h')
# 
# pdf(file = '/home/vdenis/Documents/Faks/Diplomski/diplomska radnja/obrada/scpltmat.pdf', 
#     width = 30, height = 16.875) 
# scpltmat <- scale(rezmat[, c(2:3, 8:16)]) %>%
#     pairs()
# dev.off()
...
fit <- glm(data = rezmat, gam1_rec ~ expressions.d.a * agg_tot, family = binomial)
summary(fit) #ns
...
# fit <- glm(data = rezmat, formula = leadec_rec ~ agtotcent * botdec_rec, family = binomial, subset = gameposl >= 2)
# summary(fit) #!!!
# 
# tmpy <- expand.grid(agtotcent = rezmat$agtotcent, botdec_rec = rezmat$botdec_rec, 
#                     expressions.d.a = rezmat$expressions.d.a)
# tmpy$pred <- predict(fit, type = 'response', newdata = tmpy)
# 
# plotko <- ggplot(data = tmpy, mapping = aes(x = agtotcent, y = pred,
#                                             group = botdec_rec, colour = as.factor(botdec_rec))) + 
#         geom_smooth(se = FALSE) + theme_minimal() + 
#         xlab('BPAQ_ukupno') + ylab('Vjerojatnost nekooperativne odluke') + 
#         theme(axis.text.x = element_text(size = 11), axis.text.y = element_text(size = 11),
#         axis.title.y = element_text(size = 11, vjust = 2), axis.title.x = element_text(size = 11),
#         plot.title = element_text(size = 11, hjust = 0.5)) +
#         guides(color = guide_legend(title = 'Odluka suigrača')) +
#         scale_color_manual(values = c('0' = plava, '1' = crvena), labels = c('suradnja', 'nesuradnja'))
# plotko
# 
# ggsave(plotko, 
#        file = '/home/denis/Documents/Diplomski/diplomska radnja/diplomski.tex/leadbotplot.pdf',
...
# tmpy <- expand.grid(agper = quantile(rezmat$agper), gameposl = rezmat$gameposl, 
#                     expressions.d.a = rezmat$expressions.d.a)
# tmpy$pred <- predict(fit, type = 'response', newdata = tmpy)
# 
# plotko <- ggplot(data = tmpy, mapping = aes(x = expressions.d.a, y = pred,
#                                                         group = agper, colour = as.factor(agper))) + 
#     geom_line() + theme_minimal() + facet_grid(~ gameposl) +
#     xlab('IAT-A') + ylab('Vjerojatnost nekooperativne odluke u Igri vođe') + 
#     theme(axis.text.x = element_text(size = 11), axis.text.y = element_text(size = 11),
#     axis.title.y = element_text(size = 12, vjust = 2), axis.title.x = element_text(size = 12)) +
#     guides(color = guide_legend(title = 'BPAQ-HA')) 
# plotko
 
# ggsave(plotko, 
#        file = '/home/denis/Documents/Diplomski/diplomska radnja/diplomski.tex/lmagperposLplot.pdf',
#        device = cairo_pdf, height = 28, width = 36, unit = 'cm')
...
# tmpy <- expand.grid(agg_tot = rezmat$agg_tot, botdec_rec = rezmat$botdec_rec, 
#                     expressions.d.a = rezmat$expressions.d.a)
# 
# tmpy <- rezmat
# tmpy$pred <- predict(fit, type = 'response', newdata = tmpy)
# 
# plotko <- ggplot(data = tmpy, mapping = aes(x = agg_tot, y = pred,
#                                                         group = botdec_rec, colour = as.factor(botdec_rec))) + 
#     geom_smooth(se = FALSE) + theme_minimal() + 
#     xlab('BPAQ_ukupno') + ylab('Vjerojatnost nekooperativne odluke u bilo kojoj igri') + 
#     theme(axis.text.x = element_text(size = 11), axis.text.y = element_text(size = 11),
#     axis.title.y = element_text(size = 12, vjust = 2), axis.title.x = element_text(size = 12)) +
#     guides(color = guide_legend(title = 'odluka suigrača')) 
# plotko
...
# tmpy <- expand.grid(aggtotcent = rezmat$aggtotcent, botdec_rec = rezmat$botdec_rec, 
#                     expressions.d.a = rezmat$expressions.d.a)
# 
# tmpy <- rezmat
# tmpy$pred <- predict(fit, type = 'response', newdata = tmpy)

# plotko <- ggplot(data = tmpy, mapping = aes(x = aggtotcent, y = pred,
#                                                         group = botdec_rec, colour = as.factor(botdec_rec))) + 
#     geom_smooth(se = FALSE) + theme_minimal() + 
#     xlab('BPAQ_ukupno') + ylab('Vjerojatnost nekoop2erativne odluke u bilo kojoj igri') + 
#     theme(axis.text.x = element_text(size = 11), axis.text.y = element_text(size = 11),
#     axis.title.y = element_text(size = 12, vjust = 2), axis.title.x = element_text(size = 12)) +
#     guides(color = guide_legend(title = 'odluka suigrača')) 
# plotko
...
# fit <- glm(data = rezmat, formula =  nekoop2 ~ aggtotcent : botdec_rec, family = binomial)
# summary(fit) #ns
# 
# fit <- glm(data = rezmat, formula =  nekoop2 ~ expressions.d.a, family = binomial)
# summary(fit) #ns
# 
# fit <- glm(data = rezmat, formula =  nekoop2 ~ botdec_rec, family = binomial)
# summary(fit) #ns
# 
# fit <- glm(data = rezmat, formula =  botdec_rec ~ expressions.d.a, family = binomial)
# summary(fit) #ns
# 
# fit <- glm(data = rezmat, formula =  botdec_rec ~ aggtotcent, family = binomial)
# summary(fit) #ns
# 
# fit <- lm(data = rezmat, formula =  expressions.d.a ~ aggtotcent)
# summary(fit) #ns
# 
# rezmat$nekoop2
# rezmat$nekoop2fac <- as.factor(rezmat$nekoop2)
# 
# polycor::polyserial(x = rezmat$expressions.d.a, y = rezmat$nekoop2fac, std.err = TRUE)
# polycor::polyserial(x = rezmat$agtotcent, y = rezmat$nekoop2fac, std.err = TRUE)
# polycor::polyserial(x = rezmat$agpercent, y = rezmat$nekoop2fac, std.err = TRUE)
# 
# t.test(rezmat$expressions.d.a ~ rezmat$leadec_rec)
# t.test(rezmat$agtotcent ~ rezmat$leadec_rec)
# t.test(rezmat$agpercent ~ rezmat$leadec_rec)
